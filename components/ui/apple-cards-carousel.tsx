...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      const cardWidth = isMobile() ? 230 : 288; // (md:w-72)
      ...
      >
        ...
        <div className="relative z-40 p-8 text-left">
          <motion.p
            ...
            className="text-accent text-sm md:text-base font-medium font-sans mb-1"
          >
            {card.category}
          </motion.p>
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="text-white text-xl md:text-3xl font-bold font-sans mb-2"
          >
            {card.title}
          </motion.p>
          {card.quote && (
            <motion.p
              ...
              className="text-white/70 text-xs italic font-sans line-clamp-2"
            >
              "{card.quote}"
            </motion.p>
          )}
